...
def audit_compliance(rows: list[dict[str, Any]], runs: list[dict[str, Any]]) -> None:
    ...
    if selected_run_ids:
        ...
        # ---- AI Advisory (OpenAI) — Liability & Compliance opinion ----
        st.subheader("AI Advisory — Liability & Compliance")
        st.caption(
            "Advisory only — generated by an OpenAI agent. Does not change the "
            "deterministic routing or settlement."
        )
        advisory_path = run_dir / "llm_advisory.json"
        if st.button("Generate AI advisory", key=f"advisory_{selected_claim}"):
            with st.spinner("Calling the OpenAI advisory agent…"):
                try:
                    ...
                    load_dotenv()
                    AgentLLMAdvisory().process(run_dir)
                    clear_cache()
        ...
        if advisory_path.is_file():
            adv = read_json(str(advisory_path))
            if adv.get("status") == "ok":
                lia = adv.get("liability") or {}
                comp = adv.get("compliance") or {}
                adv_left, adv_right = st.columns(2)
                with adv_left:
                    st.markdown("**Liability**")
                    st.write(f"At-fault party: {lia.get('at_fault_party', 'n/a')}")
                    st.write(
                        f"Insured {lia.get('insured_liability_pct', 0)}% / "
                        f"Claimant {lia.get('claimant_liability_pct', 0)}%"
                    )
                    st.caption(str(lia.get("rationale", "")))
                with adv_right:
                    st.markdown("**Compliance**")
                    st.write(f"Framework: {comp.get('framework', 'n/a')}")
                    st.write(f"Compliant: {comp.get('compliant', 'n/a')}")
                    concerns = ", ".join(comp.get("concerns", []) or []) or "none"
                    st.caption(f"Concerns: {concerns}")
                if adv.get("narrative"):
                    st.info(adv["narrative"])
                st.caption(f"model: {adv.get('model', 'n/a')} · advisory only")
            else:
                st.warning(
                    f"Advisory unavailable: {adv.get('reason', 'unknown')}. "
                    "Set OPENAI_API_KEY in .env to enable it."
                )
        else:
            st.caption(
                "No AI advisory generated yet — click the button above "
